class RepairRequestsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @repair_request.update(repair_request_params)
        ...
        format.json { render :show, status: :ok, location: @repair_request }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to repair_requests_url, notice: 'Repair request was successfully destroyed.' }
  ...
  private
    ...
    def repair_request_params
      params.require(:repair_request).permit(:name, :phone_number, :email, :problem)
    end
end
